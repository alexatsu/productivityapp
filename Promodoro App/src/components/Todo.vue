...
const newTodo = ref("");
const newTodoTime = ref(1); // temporarely
const todos = ref([]);
...
export function useEmitTest(todo) {
  const test = Math.floor((todo.deadline - todo.startTime) / 1000);
}
...
function addTodo() {
  ...
  const deadlineInMinutes = dataNow + newTodoTime.value * 1000 * 60;
    ...
    <ul class="completed-todos">
      ...
      >
        <input
          type="checkbox"
          checked
          @click="hideCompleted('completed', index)"
        />
        {{ todo.text }}
        {{ moment(todo.deadline - todo.startTime).format("mm") }} min
